  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-white border-t border-border overflow-hidden rounded-[40px]">
      ...
      <div className="flex-1 overflow-auto custom-scrollbar relative">
        ...
        >
          <thead className="sticky top-0 z-[40] shadow-[0_1px_3px_rgba(0,0,0,0.05)] bg-[#f8fafc]">
            <tr className="bg-[#f8fafc] text-slate-800 h-10 border-b border-slate-200">
              <th
                style={{ padding: "0" }}
              ...
              {mktPivotUniqueTypes.map((type) => (
                <th
                  key={type}
                  style={{ padding: "0" }}
                  className="text-xs font-black uppercase tracking-wider text-slate-700 text-center border-r border-border bg-indigo-50/30 text-indigo-950 align-middle"
                >
              ...
              <th
                style={{ padding: "0" }}
                className="text-xs font-black uppercase tracking-wider text-rose-800 text-right bg-rose-50/50 align-middle"
              >
            ...
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              {mktPivotUniqueTypes.map((type) => (
                <th key={`filter-${type}`} className="p-1 border-r border-border bg-slate-50/50" />
              ))}
              <th className="p-1 bg-slate-50/50" />
            </tr>
          </thead>
          <tbody>
            {mktPivotRows.map((row, idx) => (
              <tr
                key={idx}
                className={`${idx % 2 === 1 ? "bg-slate-50/30" : "bg-white"} h-11 border-b border-border`}
              >
                ...
                {mktPivotUniqueTypes.map((type) => {
                  ...
                  return (
                    <td
                      key={type}
                      className="px-5 py-2.5 text-xs font-mono text-center border-r border-border font-semibold text-slate-800 bg-indigo-50/5"
                    >
                      {val === 0 ? "0" : formatMoneyVND(val)}
                    </td>
                  );
                })}
                <td className="px-5 py-2.5 text-xs font-mono font-black text-right text-rose-950 bg-rose-50/10">
            ...
            <tr className="sticky bottom-0 z-10 bg-slate-100 font-extrabold border-t-2 border-slate-300 h-12 text-slate-900 shadow-[0_-2px_6px_rgba(0,0,0,0.05)]">
              ...
              {mktPivotUniqueTypes.map((type) => {
                ...
                return (
                  <td
                    key={type}
                    className="px-5 py-3 text-xs font-mono text-center border-r border-border text-slate-950 bg-indigo-50/20 font-black"
                  >
                    {totalVal === 0 ? "0" : formatMoneyVND(totalVal)}
                  </td>
                );
              })}
              <td className="px-5 py-3 text-xs font-mono font-black text-right text-rose-900 bg-rose-50/30">
