...
export default function TimesheetSummaryPage({
  ...
  const handleSaveData = () => {
    ...
    toast.success("Đã lưu dữ liệu hiện tại.");
  };
  ...
  const handleUploadFiles = async (files: File[]) => {
    ...
    if (toProcess.length > 0) {
      const results: { file: File; allRows: Record<string, unknown>[]; rowId: string }[] = [];
      
      for (const p of toProcess) {
        try {
          await new Promise((r) => setTimeout(r, 50));
          const allRows = await parseExcelInWorker(p.file);
          results.push({ file: p.file, allRows, rowId: p.id });
        } catch (err: unknown) {
          const errMsg = err instanceof Error ? err.message : "Unknown error";
          console.error(`Error parsing ${p.file.name}:`, err);
          handleUpdateRow(p.id, "status", "error");
          toast?.error(`Lỗi đọc ${p.file.name}: ${errMsg}`);
        }
      }
      ...
      if (results.length > 0) {
        updateAppData((prev) => {
          const next = { ...prev };
          const processedIds = new Set(results.map((r) => r.rowId));

          next.Q_Roster = next.Q_Roster || [];
          next.Q_Salary_Scale = next.Q_Salary_Scale || [];
          next.Q_Staff = next.Q_Staff || [];
          next.Q_Cache = next.Q_Cache || [];

          next.Q_Roster = next.Q_Roster.filter((r: Record<string, unknown>) => !processedIds.has(String(r._rowId || "")));
          next.Q_Salary_Scale = next.Q_Salary_Scale.filter((r: Record<string, unknown>) => !processedIds.has(String(r._rowId || "")));
          next.Q_Staff = next.Q_Staff.filter((r: Record<string, unknown>) => !processedIds.has(String(r._rowId || "")));
          next.Q_Cache = next.Q_Cache.filter((r: Record<string, unknown>) => !processedIds.has(String(r._rowId || "")));

          let newRosters: Record<string, unknown>[] = [];
          let newSalaryScales: Record<string, unknown>[] = [];
          let newStaffs: Record<string, unknown>[] = [];
          let newCaches: Record<string, unknown>[] = [];

          results.forEach(({ file, allRows, rowId }) => {
            let isSalary = false,
              isStaff = false,
              isCache = false;
            ...
            if (allRows.length > 0) {
              ...
              if (headers.includes("academic price") || isSalary || headers.includes("s code")) {
                newSalaryScales = newSalaryScales.concat(allRows);
              } else if (headers.includes("bank account number") || isStaff) {
                newStaffs = newStaffs.concat(allRows);
              } else if (headers.includes("today") || isCache) {
                newCaches = newCaches.concat(allRows);
              } else {
                const mappedRosters = allRows.map((r: Record<string, unknown>) => mapExcelRosterRow(r, file.name));
                newRosters = newRosters.concat(mappedRosters);
              }
            }
          });

          next.Q_Roster = next.Q_Roster.concat(newRosters);
          next.Q_Salary_Scale = next.Q_Salary_Scale.concat(newSalaryScales);
          next.Q_Staff = next.Q_Staff.concat(newStaffs);
          next.Q_Cache = next.Q_Cache.concat(newCaches);

          const d = new Date();
          const timestampStr = `${d.getHours()}:${String(d.getMinutes()).padStart(2, "0")} ${d.getDate()}/${d.getMonth() + 1}`;
          
          next.Timesheet_InputList = (next.Timesheet_InputList || []).map((input) => {
            if (processedIds.has(input.id)) {
              const res = results.find((r) => r.rowId === input.id);
              if (res) {
                return {
                  ...input,
                  status: "success",
  ...
  return (
    <div className="page-timesheet-summary flex-1 flex flex-col min-h-0 bg-transparent p-8 gap-8 items-center overflow-auto custom-scrollbar">
      ...
      <div className="bg-white soft-card force-light flex-1 flex flex-col min-h-0 w-full relative overflow-hidden rounded-[54px] border border-rose-100 shadow-sm">
        <div className="absolute inset-0 bg-pattern-rose opacity-[0.05] pointer-events-none" />

        <div className="p-8 flex flex-col md:flex-row items-center justify-between gap-6 bg-rose-50/10 shrink-0 border-none relative z-10 overflow-hidden rounded-t-[54px] rounded-b-none">
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <h2 className="text-3xl font-normal font-serif text-foreground tracking-tight flex items-end gap-1" style={{ fontSize: "25px", paddingTop: "0px", paddingBottom: "0px" }}>
                  ...
                  <span className="text-3xl font-normal font-serif text-foreground tracking-tight ml-1" style={{ lineHeight: "33px" }}>
                    center
                  </span>
                </h2>
              </div>
