...
export function BulkPayment({
  ...
  const handleGenerateReport = useCallback(async () => {
    ...
    try {
      ...
      let msg = `TỔNG CỘNG BẢNG KÊ: ${formatMoneyVND(reportTotal)} `;
      msg += isTotalMatch
        ? "✅ KHỚP VỚI NGUỒN"
        : `❌ LỆCH BAN ĐẦU ${formatMoneyVND(reportTotal - bankNorthTotal)}`;

      msg += ` | PHÉP ĐỐI CHIẾU: (Sheet 1 + Hold) ${formatMoneyVND(sheet1Total)} + ${formatMoneyVND(holdTotal)} = ${formatMoneyVND(calculatedTotal)}`;
      msg += isSourceMatch
        ? ` (✅ KHỚP DỮ LIỆU GỐC)`
        : ` (❌ LỆCH DỮ LIỆU GỐC: Chênh lệch: ${formatMoneyVND(Math.abs(calculatedTotal - bankNorthTotal))})`;
          ...
          <AnimatePresence>
            {validationMsg && !isGenerating && (
              <motion.div
                ...
                className={`p-6 border-l-4 rounded-2xl relative z-20 ${isSuccess ? "border-emerald-400 bg-emerald-50/20" : "border-amber-400 bg-amber-50/20"}`}
              >
                <h4
                  className={`text-[0.625rem] font-bold mb-4 uppercase flex items-center gap-2 tracking-[0.2em] ${isSuccess ? "text-emerald-700" : "text-amber-700"}`}
                >
                  {isSuccess ? (
                ...
                <div className="space-y-3 text-[0.7rem] font-medium leading-relaxed italic font-serif text-foreground/80">
        ...
        {appData.BankExport.data.length === 0 ? (
            ...
            <div className="flex-1 min-h-0 bg-white relative z-10 flex flex-col">
              <DataTable
                ...
                showFooter={true}
                hideSearch={true}
                headerClassName="bg-[#F3EFE0] text-[0.82rem] font-bold uppercase tracking-[0.12em] text-[#2d1912]"
              />
            </div>
          )}
      </div>
