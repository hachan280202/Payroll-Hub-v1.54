        ...
        if (activeTab === "Hold_AE" && header === "Nghiệp vụ") {
          renderOption = (value: any, row: any) => {
            ...
            const buttonClass = (
              ...
              const base =
                "flex items-center gap-1.5 px-2 py-1.5 flex-1 justify-center rounded-lg text-[0.65rem] uppercase tracking-wider transition-all";
              if (!isPeriodMatch) {
                return `${base} opacity-30 grayscale cursor-not-allowed pointer-events-none ${active ? activeStyle : inactiveStyle}`;
              }
              return `${base} cursor-pointer ${
                active
                  ? `${activeStyle} shadow-md font-extrabold scale-[1.02] z-10`
                  : `${inactiveStyle} opacity-60 shadow-inner font-medium hover:opacity-100 hover:bg-slate-100`
              }`;
            };

            const titleAdd = !isPeriodMatch
              ? `Chỉ sửa đổi được tại card tháng chọn: ${rowReportingMonth}`
              : "Chuyển thành Add / Trả tiền";
            ...
            return (
              <div
                ...
                className="flex items-center justify-center gap-1 w-full min-w-[200px]"
              >
                <button
                  onClick={() => {
                ...
                <div className="flex items-center gap-5 relative z-10 shrink-0">
                  <div className="w-[40px] h-[40px] bg-white rounded-[400px] bg-transparent shrink-0 hidden md:block relative overflow-hidden">
                    <img
                      src={appLogo}
                      alt="Logo"
                      className="w-full h-full object-contain rounded-[400px]"
                      style={{
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-primary/10 p-12">
                    <div className="w-28 h-28 bg-white mb-8 relative">
                      <img
                        src={appLogo}
                        alt="Logo"
                        className="w-full h-full object-cover opacity-30 grayscale rounded-[30px] border-solid border-primary/20 scale-[1.3]"
                        style={{
                          imageRendering: "-webkit-optimize-contrast",
                          borderWidth: "10px",
                        }}
