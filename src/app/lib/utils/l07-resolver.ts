...
export function resolveL07Logic(
  ...
  if (isMktLocal) {
    if (
      ...
      const mktMapped = getL07FromChargeToCenterMkt(originalL07);
      if (mktMapped && mktMapped !== "MKT LOCAL NORTH" && mktMapped !== "MKT LOCAL SOUTH") {
        chargeToCenterMkt = mktMapped;
      } else {
        chargeToCenterMkt = originalL07;
      }
    } else if (rawChargeToCenter) {
      if (
        rawChargeToCenter.toUpperCase() === "MKT NORTH" ||
        rawChargeToCenter.toUpperCase() === "MKT LOCAL NORTH"
      ) {
        chargeToCenterMkt = "MKT LOCAL NORTH";
      } else if (rawChargeToCenter.toUpperCase() === "NTW") {
        chargeToCenterMkt = "NTW";
      } else if (
        rawChargeToCenter.toUpperCase() === "MKT SOUTH" ||
        rawChargeToCenter.toUpperCase() === "MKT LOCAL SOUTH"
      ) {
        chargeToCenterMkt = "MKT LOCAL SOUTH";
      } else {
        const mktMapped = getL07FromChargeToCenterMkt(rawChargeToCenter);
        if (mktMapped && mktMapped !== "MKT LOCAL NORTH" && mktMapped !== "MKT LOCAL SOUTH") {
          chargeToCenterMkt = mktMapped;
        } else {
